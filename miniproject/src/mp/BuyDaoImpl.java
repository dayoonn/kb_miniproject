...
public class BuyDaoImpl implements BuyDao {
	...
	public void add(BuyDto dto) throws SQLException, RecordNotFoundException {
        ...
        try {
        	//등록 여부 검사
        	if (findById(dto.getBUY_NUM())!= null) {//해당 아이디가 이미 있다면 
            ...
            pstmt.setInt(7, dto.getBUY_PRICE());
            // 6. SQL 전송, 결과수신
	...
	public void update(BuyDto dto) throws SQLException, RecordNotFoundException {
        ...
        try {
        	//등록 여부 검사
        	if (findById(dto.getBUY_NUM())== null) {//해당 아이디가 없다면
        		throw new RecordNotFoundException(dto.getBUY_NUM()+"는 없음");//throw니까 메서드가 멈추고 메세지 출력 
        	}
            con = JdbcUtil.connect();//연결 전 등록 여부 검사 
            // 3. SQL 작성
            String sql = "UPDATE BUY SET BUY_NUM=?,PAY_ID=?,PRODUCT_ID=?,PRODUCT_PRICE=?,BUY_QUANTITY=?,BUY_PRICE=? ";
            sql += "WHERE no = ? ";
            // 4. Statement 생성
            pstmt = con.prepareStatement(sql);
            // 5. 데이터 설정
	...
	public void delete(int no) throws SQLException, RecordNotFoundException {
        ...
        try {
        	//등록 여부 검사
        	if (findById(no)== null) {//해당 아이디가 없다면
	...
	@Override
	public BuyDto findById(int no) throws SQLException, ClassNotFoundException {
		BuyDto dto = null;
    	Connection con = null;
        PreparedStatement pstmt = null;
        try {
